# Ansible Automation scenario Provisioner

The `github.com/ansible/scenarios` contains an Ansible Playbook `provision_lab.yml`, which is an automated lab setup for Ansible training on AWS (Amazon Web Services).  Set the `scenario_type` variable below to provision the corresponding scenario.

| scenario | scenario Type Var   |
|---|---|
| Ansible for Red Hat Enterprise Linux scenario | `scenario_type: rhel`  |
| Ansible for Red Hat Enterprise Linux scenario - 90 minutes  | `scenario_type: rhel_90`    |
| Ansible Network Automation scenario | `scenario_type: network`  |
| Ansible F5 scenario | `scenario_type: f5`   |
| Ansible Security Automation | `scenario_type: security`   |
| Ansible Windows Automation  | `scenario_type: windows`    |
| Ansible Demo Mode  | `scenario_type: demo`    |
| Smart Management scenario | `scenario_type: smart_mgmt` |

## Table Of Contents

<!-- TOC titleSize:2 tabSpaces:2 depthFrom:1 depthTo:6 withLinks:1 updateOnSave:1 orderedList:0 skip:0 title:1 charForUnorderedList:* -->
## Table of Contents
* [Ansible Automation scenario Provisioner](#ansible-automation-scenario-provisioner)
  ...
  * [Ansible-Navigator](#ansible-navigator)
    ...
    * [Setup (per scenario)](#setup-per-scenario)
...
If you are going to use ansible-navigator and the scenario execution environment there are two (2) differences from ansible-playbook method used previously:
...
# Set the right scenario type, like network, rhel or f5 (see above)
scenario_type: rhel
...
# forces ansible.scenarios collection to install latest edits every time
developer_mode: true
...
**How do you use the manifest.zip with the scenario?**

There are currently two ways to integrate your license file with the scenario:

1. Put the manifest.zip file into provisioner folder

  The first way is to make sure your license/manifest has the exact name `manifest.zip` and put it into the same folder as the `provision_lab.yml` playbook (e.g.) `<your-path>/scenarios/provisioner/manifest.zip`

2. Turn the manifest.zip into a variable

  The second way is to turn the `manifest.zip `into a base64 variable.

  This allows the `manifest.zip` to be treated like an Ansible variable so that it can work with CI systems like Github Actions or Zuul.  This also makes it easier to work with Automation controller, in case you are spinning up a scenario using Automation controller itself.
...
* [sample-vars-rhel.yml](sample_scenarios/sample-vars-rhel.yml) - example for the Ansible RHEL scenario
* [sample-vars-windows.yml](sample_scenarios/sample-vars-windows.yml) - example for the **Ansible Windows scenario**
* [sample-vars-network.yml](sample_scenarios/sample-vars-network.yml) - example for the **Ansible Network scenario**
* [sample-vars-f5.yml](sample_scenarios/sample-vars-f5.yml) - example for **Ansible F5 scenario**
* [sample-vars-tower-auto.yml](sample_scenarios/sample-vars-tower-auto.yml) - example for Tower installation and licensing
* [sample-vars-rhel-90.yml](sample_scenarios/sample-vars-tower-auto.yml) - example for Tower installation and licensing
* [sample-vars-rhel-90.yml](sample_scenarios/sample-vars-rhel-90.yml) - example for `rhel_90` scenario, meant to be taught in 90 minutes
* [sample-vars-demo.yml](sample_scenarios/sample-vars-demo.yml) - example for `demo` mode, aggregate of all scenario topologies
* [sameple-vars-smart_mgmt.yml](sample_scenarios/sample-vars-smart_mgmt.yml) - example for `smart_mgmt` scenario. [Read Notes](#smart-management)
...
* Workbench information is stored in two places after you provision:
  ...
  * in a local directory named after the scenario (e.g. testscenario/instructor_inventory)
  * By default there will be a website `ec2_name_prefix.scenario_dns_zone` (e.g. `testscenario.rhdemo.io`)

    * **NOTE:** It is possible to change the DNS domain (right now this is only supported via a AWS Route 53 Hosted Zone) using the parameter `scenario_dns_zone` in your `extra_vars.yml` file.
...
* The instructor can see all assigned students and what their workbench is by visiting `ec2_name_prefix.scenario_dns_zone/list.php` (e.g. `testscenario.rhdemo.io/list.php`)
...
This means that each student workbench will get an individual DNS entry.  For example a DNS name will look like this: `https://student1.testscenario.rhdemo.io`

* **NOTE:** The variable `dns_type` defaults to `aws`.  This can also be set to `dns_type: none`.
* **NOTE:**  If Lets Encrypt fails, the scenario provisioner will still pass, and alert you of errors in the `summary_information` at the end of the `provision_lab.yml` Ansible Playbook.
...
The Smart Management Lab also requires AWS DNS to be enabled. See [sample vars](./sample_scenarios/sample-vars-smart_mgmt.yml) for required configuration.

## Developer Mode and understanding collections

The Ansible scenarios are actually a collection.  Every role is called using the FQCN (fully qualified collection name).  For example to setup the control node (e.g. install Automation controller) we call the role
...
If you want to contribute to the scenarios, check out the [contribution guide](../docs/contribute.md).
...
Which will install all demos onto the Ansible Tower instance.  Not all demos will work on any `scenario_type`.  Please refer to the [Demo repository list](https://github.com/ansible/product-demos#demo-repository).
...
* Creation of an SSH key pair (stored at ./scenarioNAME/scenarioNAME-private.pem)
...
Please [file issues on Github](https://github.com/ansible/scenarios/issues).  Please fill out all required information.  Your issue will be closed if you skip required information in the Github issues template.

![Ansible-scenario-Logo.png](../images/Ansible-scenario-Logo.png)
